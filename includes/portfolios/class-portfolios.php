	...
	class NTH_Portfolio {
		...
		public function addImageSize(){
			...
			if( class_exists( 'NTH_Plugin_Panel' ) ) {
				$options = NTH_Plugin_Panel::get_option();
				$options = isset( $options['portfolio_thumb'] )? $options['portfolio_thumb']: array();
				if( empty($options['crop']) ) $options['crop'] = 0;
