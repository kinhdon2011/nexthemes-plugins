	...
	class NTH_StaticBlock{
		...
		public static function getStaticBlocks( $cats = array('all') ){
			...
			$args = array(
				...
				'meta_query' => array(
					array(
						'key'	=> THEME_SLUG.'_nth_stblock_options',
						'value'	=> $cats,
						'compare' => "IN"
					),
				),
			...
			//$blocks = get_posts( array( 'post_type' => 'nth_stblock', 'posts_per_page' => 50) );
		...
		public function tableContent( $column_name, $post_id ){
			if( strcmp( trim( $column_name ), 'position' ) == 0 ) {
				$data = get_post_meta($post_id, THEME_SLUG.'_nth_stblock_options',true);
