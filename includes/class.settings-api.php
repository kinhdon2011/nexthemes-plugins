...
class WeDevs_Settings_API {
	...
	function callback_image_size( $args ){
		...
		$html .= sprintf( '&nbsp; <label><input type="checkbox" name="%1$s[%2$s][crop]" id="%1$s[%2$s][crop]" value="1" %3$s /> %4$s</label>', $args['section'], $args['id'], checked( $value['crop'], '1', false ), __('Hard Crop?', 'nexthemes-plugin') );
    ...
    function show_navigation() {
        ...
        foreach ( $this->settings_sections as $tab ) {
            ...
            $html .= sprintf( '<a href="#%1$s" class="nav-tab %3$s" id="%1$s-tab">%2$s</a>', $tab['id'], $tab['title'], $class );
        }
